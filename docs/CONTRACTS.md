...
G — Goal:
- Prove one real Repository Ready branch and pull request with least privilege, exact repository authorization, idempotent recovery, and no default-branch or merge authority.
...
T — Task:
- Future implementation task: T-022, only after this contract is explicitly accepted and one dogfood repository is named.
...
S — Scope:
- Future implementation allowed after acceptance:
  - credential-provider and live GitHub branch/PR/check adapters behind existing ports;
  - mutation-guard and exact repository-allowlist enforcement;
  - mocked contract tests, local bare-remote regression, and one separately approved live dogfood repository;
  - task-scoped docs, evidence, and recovery state.
- Forbidden:
  - default-branch mutation or force push;
  - automatic merge, branch deletion, PR close, or cleanup mutation;
  - `.github/workflows/**`, repository metadata/settings, rulesets, secrets, environments, releases, deploys, Pages, external publication, or M2-M6;
...
V — Verify:
- Current Light Rail acceptance: contract and coordinate checks pass; Blueprint remains healthy; official GitHub documentation supports the permission/credential claims; manual acceptance is explicitly pending.
...
X — Stop:
- Stop before implementation or credential access until the user accepts the four Required User Decisions and names the exact dogfood repository.
...
P — Persist:
- Current draft: CONTRACTS, TASKS, BLUEPRINTS, HARNESS, HANDOFF, TRACE/index/status.
- Future implementation: accepted task, threat/permission decision, credential-source metadata without secrets, allowlist identity, idempotency/effect evidence, and live verification state.

### Credential Contract

- Initial dogfood credential: a short-lived fine-grained personal access token restricted to the one allowlisted repository. GitHub recommends fine-grained tokens over classic PATs and recommends minimum permissions plus expiration.
- Accepted runtime source: `GH_TOKEN` or an injected in-memory credential provider. The token value must never enter plans, traces, logs, errors, snapshots, command history generated by gh-polish, or project files.
- `GITHUB_TOKEN` is not an implicit local credential; classic PATs and SSH-agent ambient authority fail closed unless separately contracted.
- Recommended maximum dogfood expiry: 7 days; revoke immediately after dogfood or on any suspected exposure.
- Minimum repository permissions for the initial slice:
  - `Contents: write` for creating/pushing the non-default branch;
  - `Pull requests: write` for creating or reusing the PR;
  - `Actions: read` for exact branch/SHA workflow-run evidence.
- `Workflows: write` is deliberately absent. Any artifact targeting `.github/workflows/**` is excluded before live execution.
- Official references:
  - [Git references permissions](https://docs.github.com/en/rest/git/refs)
  - [Pull request permissions](https://docs.github.com/en/rest/pulls/pulls)
  - [Workflow-run permissions](https://docs.github.com/en/rest/actions/workflow-runs)
  - [Credential security](https://docs.github.com/en/rest/authentication/keeping-your-api-credentials-secure)
...
- All three independent gates are mandatory:
  ...
  2. exact `GH_POLISH_LIVE_REPOSITORY=owner/repository` match;
  3. a valid T-020 review token plus an explicit live-run confirmation naming that repository and the exact effect IDs.
- Redirects, renamed/transferred repositories, forks, remote mismatch, repository-ID mismatch, default-branch drift, stale base SHA, or missing permissions stop before mutation.
- Recommended dogfood target: a dedicated, non-sensitive private sandbox repository with an initial commit and no production deployment or protected data.

### Live Effect Contract

- Initial allowlisted paths may include only missing `README.md`, `.gitignore`, `CONTRIBUTING.md`, and `.github/pull_request_template.md` selected as `create` effects.
- Existing paths, symlinks, manual-review items, unknown items, deletes, renames, binary files, submodules, workflow files, and paths outside the repository are refused.
- Deterministic head branch: `gh-polish/live/<first-12-plan-digest>` from the accepted base SHA.
- The pull request is created as a draft by default, targets the fetched default branch, includes the plan ID/digest and effect IDs in a machine-readable marker, and is never merged automatically.

### Idempotency and Recovery Contract

- Idempotency key: repository numeric ID + plan digest + base branch + base SHA + deterministic head branch.
- Retry preflight classifies each stage as absent, matching, conflicting, or completed:
  - matching branch at the expected pushed SHA is reused;
  - an existing PR with the same repository, base, head, and plan marker is reused whether open or closed and is never duplicated;
  - a branch at an unexpected SHA, mismatched PR marker/base/head, multiple matching PRs, or a changed default/base stops with conflict evidence;
  - checks are read only for the exact pushed head SHA; unrelated or older runs cannot satisfy completion.
- Partial failure records branch, push, PR, and check stages separately. Retry begins at the first unfinished stage and never repeats a completed mutation.
- `422` creation conflicts trigger read-after-write reconciliation before retry; permission/rate/network failures preserve evidence and return one next action.
- No compensation mutation is automatic. The default cleanup policy leaves the draft PR and branch intact for human inspection.

### Threat Boundary

| Threat | Required control |
|---|---|
| token disclosure | in-memory only, redaction, no persistence/logging, short expiry, documented revocation |
| confused-deputy or cross-repository write | exact repository name + numeric ID + local remote + saved artifact agreement |
| stale or reconstructed plan | digest, expiry, base SHA, content hashes, review token, and exact effects revalidated |
| workflow/permission escalation | no workflow effects and no `Workflows: write` permission |
| branch hijack or force update | deterministic branch, expected-SHA comparison, no force push, conflict stops |
| duplicate PR on retry | deterministic idempotency key, plan marker, read-before/create and read-after-conflict reconciliation |
| false completion | exact head-SHA checks and separate pending/failure/permission-limited evidence |
| accidental merge or cleanup | no merge, close, delete-branch, or default-branch endpoint in the adapter surface |

### Future Implementation Verification
- TDD mode: required for the future implementation.
- Red tests before implementation: missing enable flag; absent/wrong allowlist; repository-ID/default/base mismatch; token absent/redaction; insufficient permissions; forbidden path/workflow; invalid review token; branch conflict; existing matching PR reuse; mismatched PR conflict; `422` reconciliation; network/permission partial failure; exact-SHA checks; retry without duplicate branch/push/PR.
- Mandatory non-live Green: injected fetch/Git doubles and local bare remotes cover all contract states without credentials.
- Mandatory live dogfood, only after separate repository approval: one reviewed create-only plan produces or reuses one deterministic non-default branch and one draft PR; default branch remains unchanged; exact-SHA checks and recovery evidence are recorded; no merge/cleanup occurs.
- Secret scan covers stdout, stderr, thrown errors, traces, snapshots, Git config, plan store, and project files.
- Project CI, Blueprint/SEC, CodeRail Full Rail gates, and before/after repository evidence must pass.

### Future Stop Conditions
- Stop before implementation until the user accepts this contract and supplies the exact dogfood `owner/repository` plus credential authority.
- Stop live execution on any identity/allowlist mismatch, stale artifact/base, dirty or unexpected local state, forbidden effect, insufficient/unclear permission, branch/PR conflict, token exposure, rate-limit ambiguity, or docs/code contradiction.
- Pending checks return `stage-complete`/`not-ready`; they do not justify merge or live M1 achieved.

### Future Persistence
- Accepted contract and task Coordinate; threat/permission blueprint; credential-source metadata without secrets; repository allowlist identity; plan/review/idempotency keys; per-stage live evidence; TASKS, HANDOFF, DECISIONS, ASSETS, HARNESS, TRACE/index/status.
- Never persist the token value.
